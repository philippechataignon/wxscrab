...
char *load_uncompressed(const char *file_name, unsigned int *dic_size)
{
	...
	if (r < *dic_size) {
		...
		printf("\n");
		printf
		    ("** The number of bytes read is less than the size of the file **\n");
		printf
		    ("** this may be OK if you run a Microsoft OS but not on Unix   **\n");
		printf
...
{
	strncpy(header->ident, _COMPIL_KEYWORD_, sizeof( _COMPIL_KEYWORD_));
